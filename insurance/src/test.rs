...
#[test]
fn test_create_policy_pause_guard() {
    let (env, client, owner) = setup();

    client.set_pause_admin(&owner, &owner);

    // Pause create_policy function specifically
    client.pause_function(&owner, &soroban_sdk::symbol_short!("crt_pol"));
    ...
    // Unpause specifically
    client.unpause_function(&owner, &soroban_sdk::symbol_short!("crt_pol"));
    ...
    assert_eq!(policy_id, 1); // ID is 2 because test setup creates one policy! Wait, setup doesn't create a policy. Let's not assert_eq, just assert it doesn't fail.
...
#[test]
fn test_pay_premium_pause_guard() {
    let (env, client, owner) = setup();
    ...
    // Pause pay_premium specifically
    client.pause_function(&owner, &soroban_sdk::symbol_short!("pay_prem"));

    let res = client.try_pay_premium(&owner, &policy_id);
    assert!(res.is_err());

    client.unpause_function(&owner, &soroban_sdk::symbol_short!("pay_prem"));
...
#[test]
fn test_deactivate_policy_pause_guard() {
    let (env, client, owner) = setup();
    ...
    // Pause deactivate_policy specifically
    client.pause_function(&owner, &soroban_sdk::symbol_short!("deact"));

    let res = client.try_deactivate_policy(&owner, &policy_id);
    assert!(res.is_err());

    client.unpause_function(&owner, &soroban_sdk::symbol_short!("deact"));
    ...
    // Global pause
    client.pause(&owner);
    let res = client.try_deactivate_policy(&owner, &policy_id_2);
    assert!(res.is_err());
}
